...
async function cercar_video(args, message) {
    ...
    if (ytdl.validateURL(args[0])) {
        ...
        const video_finder = async (query) => {
            try {
                const video_result = await ytSearch({search: query, category: 'music', pages: 1});
                return video_result.videos.length > 1 ? video_result.videos[0] : null;
            } catch (err) {
                console.error(err);
                enviarError(message, "Error al cercar la cançó", err);
                return null;
            }
        };
...
async function video_player(guild, song, voice_channel_name) {
...
async function enviarError(message, str, err) {
