...
const { getColorFromCommand, shuffleArray, getRandomFromArray } = require('./common.js');
...
function sortir_i_esborrar(song_queue, guild, abandonat, kicked = false) {
  const embed = new Discord.MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`👋 Adeu!`);

  if (song_queue && kicked) {
    embed.setDescription(`Desconnectant del canal de veu ${song_queue.voice_channel} ja que s'ha fet fora del canal.`);
  } else {
    if (abandonat) {
      embed.setDescription(
        `Desconnectant del canal de veu ${song_queue.voice_channel} ja que no hi ha ningú més al canal.`
      );
    } else {
      embed.setDescription(
        `Desconnectant del canal de veu ${song_queue.voice_channel}${
          song_queue.stopping ? '' : ` després de ${DISCONNECTION_DELAY_SECONDS}s d'inactivitat`
        }`
      );
    }
  }
...
async function video_player(guild, song, voice_channel_name, seekTime) {
  ...
  if (!song) {
    if (song_queue.stopping) sortir_i_esborrar(song_queue, guild, false);
    else
...
async function enviarErrorChannel(channel, str, err, command = 'ni idea') {
  const errorEmbed = new Discord.MessageEmbed()
    .setColor(0xff0000) // Red
    .setTitle('⚠️ Alguna cosa ha anat malament! ⚠️')
    .setDescription(str)
...
module.exports = {
    ...
    if (!server_queue) {
      ...
      try {
        ...
        queue_constructor.connection
          .on('disconnect', () => {
            sortir_i_esborrar(queue.get(message.guild.id), message.guild, false, true);
          })
          .on('error', (err) => {
            enviarError(message, 'Hi ha hagut un error amb la connexió', err, '(VoiceConnection que ve del play)');
          });
    ...
    if (!server_queue) {
      ...
      try {
        ...
        queue_constructor.connection
          ...
          .on('error', (err) => {
            enviarError(message, 'Hi ha hagut un error amb la connexió', err, '(VoiceConnection que ve del playnow)');
          });
        video_player(message.guild, queue_constructor.songs[0], voice_channel.name, 0);
      } catch (err) {
        queue.delete(message.guild.id);
    ...
    if (!server_queue) {
      ...
      try {
        ...
        queue_constructor.connection
          ...
          .on('error', (err) => {
            enviarError(message, 'Hi ha hagut un error amb la connexió', err, '(VoiceConnection que ve del playnext)');
          });
        video_player(message.guild, queue_constructor.songs[0], voice_channel.name, 0);
      } catch (err) {
        queue.delete(message.guild.id);
  ...
  show_list: (message, server_queue, args) => {
    ...
    let embed = new Discord.MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`🎵 **${songs[0].title}** 🎵`);
  ...
  resume_song: (message, server_queue) => {
    ...
    let embed = new Discord.MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('⏯️ Reprenent...');
    message.channel.send(embed);
  },
  ...
  switch_loop: (message, server_queue) => {
    ...
    let embed = new Discord.MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`🔁 ${paraula} mode bucle...`);
    message.channel.send(embed);
  },
  ...
  seekSecond: (message, server_queue, voice_channel, second) => {
    ...
    if (second > current.duration) {
      return message.channel.send(
        '**❌ Error: El segon que vols anar està fora de la cançó! Ves amb compte amb la duració.**'
      );
    } else if (second < 0) {
