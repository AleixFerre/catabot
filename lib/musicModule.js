...
const VIDEO_MAX_DURATION = 60 * 60 * 2; // 2h in seconds
...
Mira si és publica i que no provingui de \`music.youtube.com\``
    ...
    if (server_queue) {
      if (server_queue.timeout) {
        ...
        if (server_queue?.connection?.dispatcher && server_queue?.songs) {
          const streamSeconds = server_queue.connection.dispatcher.streamTime / 1000;
  ...
  clear_list: (message, server_queue, args) => {
    ...
    let embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(title);
    return message.channel.send(embed);
  },
