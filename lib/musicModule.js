...
async function connectToChannel(voice_channel, queue_constructor, message) {
  ...
  queue_constructor.connection
    .on('disconnect', () => {
      esborrar(queue.get(message.guild.id), message.guild);
    })
    .on('error', (err) => {
...
function esborrar(song_queue, guild) {
...
async function enviarError(message, str, err, command = 'ni idea') {
  enviarErrorChannel(message.channel, str, err, command);
}
...
async function enviarErrorChannel(channel, str, err, command = 'ni idea') {
  ...
  const msg = await channel.send(errorEmbed);
