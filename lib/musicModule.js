...
async function cercar_video(args, message) {
	...
	if (ytdl.validateURL(args[0])) {
		...
		const song_info = await ytdl.getInfo(args[0]).catch(() => {
			hasError = true;
			message.channel.send('**❌ Error: Aquest video no està disponible! Prova amb un altre.**');
		});
...
async function video_player(message, song, voice_channel_name, seekTime, tries = 0) {
	...
	try {
		...
		const stream = await ytdl(song.url, {
			...
			hasError = true;
			return enviarError(message, "Error al cercar l'audio de l'enllaç", err, 'ytdl');
		});
		...
		song_queue.connection
			...
			.on('start', () => {
				...
				if (!song_queue.silent) {
					const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`🎶 Està sonant: **${song.title}**`);
...
module.exports = {
	...
	play_song: async (message, args, server_queue, voice_channel, prefix) => {
		...
		if (song.duration > VIDEO_MAX_DURATION) {
			return message.channel.send('**❌ Error: No es poden reproduir videos de més de 2h.**');
		}
...
Mira si és publica i que no provingui de \`music.youtube.com\``,
	...
	skip_song: (message, server_queue) => {
		...
		if (!server_queue || !server_queue.connection || !server_queue.connection.dispatcher) {
			const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('No hi ha cap cançó a la cua 😔');
			return message.channel.send(embed);
		}
	...
	clear_list: (message, server_queue, args) => {
		...
		const title = n === 1 ? '🗑️ Esborrada 1 cançó correctament!' : `🗑️ Esborrades ${n} cançons correctament!`;
	...
	show_list: (message, server_queue, args) => {
		...
		const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`🎵 **${songs[0].title}** 🎵`);
	...
	show_np: (message, server_queue) => {
		...
		if (!server_queue || server_queue.timeout) {
			const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('No està sonant cap cançó 😔');
			return message.channel.send(embed);
		}
	...
	resume_song: (message, server_queue) => {
		...
		const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('⏯️ Reprenent...');
		message.channel.send(embed);
	},
	...
	switch_loop: (message, server_queue) => {
		...
		const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`🔁 ${paraula} mode bucle...`);
		message.channel.send(embed);
	},
	...
	set_volume: (message, server_queue, newVolume) => {
		...
		const embed = new MessageEmbed()
			.setColor(getColorFromCommand(TYPE))
			.setTitle(`🔊 Nou volum: ${newVolume === 5000 ? 'EARRAPE' : `${newVolume}%`}`);
	...
	shuffle_list: (message, server_queue) => {
		...
		const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('🔀 Llista barrejada correctament!');
		return message.channel.send(embed);
	},
	...
	invert_list: (message, server_queue) => {
		...
		const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle('🙃 Llista invertida correctament!');
		return message.channel.send(embed);
	},
	...
	showSessionCount(message) {
		const embed = new MessageEmbed()
			.setColor(getColorFromCommand(TYPE))
			.setTitle(`👍 Hi ha ${queue.size} ${queue.size === 1 ? 'sessió' : 'sessions'} actualment!`);
		message.channel.send(embed);
	},
};
