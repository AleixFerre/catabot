...
async function video_player(message, song, voice_channel_name, seekTime) {
...
async function enviarError(message, str, err, command = 'ni idea') {
  ...
  let owner = await msg.client.users.fetch(process.env.IdOwner);
...
module.exports = {
  ...
  play_song: async (message, args, server_queue, voice_channel, prefix) => {
    ...
    if (!server_queue) {
      ...
      video_player(guild, server_queue.songs[0], server_queue.voice_channel.name, 0);
    } else {
  ...
  playnow_song: async (message, args, server_queue, voice_channel, prefix) => {
    ...
    if (!server_queue) {
      ...
      try {
        await connectToChannel(voice_channel, queue_constructor, message);
        video_player(guild, queue_constructor.songs[0], voice_channel.name, 0);
      } catch (err) {
        queue.delete(message.guild.id);
...
Mira si és publica i que no provingui de \`music.youtube.com\``,
    ...
    if (willPlayNow) {
      try {
        await connectToChannel(voice_channel, queue_constructor, message);
        video_player(guild, server_queue.songs[0], voice_channel.name, 0);
      } catch (err) {
        queue.delete(message.guild.id);
  ...
  seekSecond: (message, server_queue, voice_channel, second) => {
    ...
    video_player(guild, current, voice_channel.name, second);
  },
};
