...
const utilsModel = require('../models/utilsSchema.js');
...
module.exports = {
  ...
  deleteUsersFromServer: async (serverID) => {
    return (await userModel.deleteMany({ 'IDs.serverID': serverID })).deletedCount;
  },
  ...
  setLastForceReset: async (userID, channelID) => {
    console.log(`${userID} set the last force reset to ${channelID}`);
  ...
  resetLastForceReset: async () => {
    console.log('Esborrant el last force reset...');
    await utilsModel.updateOne(
      {}, // First ocurrence of the document
      {
        lastForceReset: null,
