...
async function showChampStats(champName) {
	...
	const info = `• Attack: \`${champ.info.attack}\`
• Deffense: \`${champ.info.defense}\`
• Magic: \`${champ.info.magic}\`
• Difficulty: \`${champ.info.difficulty}\``;

	const stats = `• HP: \`${champ.stats.hp}\`
• Mana: \`${champ.stats.mp}\`
...
• Attack Speed: \`${champ.stats.attackspeed}\``;
	...
	const embed = new MessageEmbed().setColor(getColorFromCommand(TYPE)).setTitle(`**${champ.name}, ${champ.title}**`);
...
async function showSpellStats(spellName) {
	...
	const predictedSpell = predict(spellName, names);
...
async function showItemStats(itemName) {
	const filteredItems = Object.values(items).filter(
		(i) => distanciaEdicio(i.name.toLowerCase(), itemName.toLowerCase()) < 3,
	);
...
module.exports = {
	...
	async execute(message, args, server) {
		...
		if (commandType === 'champ' || commandType === 'champion' || commandType === 'champs') {
			...
			const predictedName = predictChamp(theName);
			if (predictedName === '-1') {
