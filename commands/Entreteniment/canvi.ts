...
const conversions = require('../../storage/conversions.json');
...
module.exports = {
	...
	async execute(message, args, server) {
		...
		const canviAUSD = conversions[args[1].toUpperCase()];
		const canviDeUSD = conversions[args[2].toUpperCase()];
		const res = ((parseFloat(args[0]) * canviAUSD) / canviDeUSD).toFixed(3);
