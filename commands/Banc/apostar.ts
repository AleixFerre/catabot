...
module.exports = {
	...
	async execute(message, args, server) {
		let amount = 0;
		let multiplier = 1; // Per defecte x1
		...
		const coin = Math.round(Math.random() * multiplier); // We round between 0:n so we have randomly true or false
		if (coin === 1) {
			// Guanyem
			amount *= multiplier; // Multipliquem per m per qui hagi apostat multiplicant
			user.money += parseInt(amount);
			content = message.author.username + ' has guanyat😆!\n💰' + amount + ' monedes afegides a la teva conta.💰';
			if (multiplicant) {
				content += '\nHas utilitzat el multiplicador x' + multiplier;
			}
		} else {
			// Perdem
			user.money -= parseInt(amount);
