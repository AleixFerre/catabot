  ...
  async execute(message, args, server) {
    ...
    async function calculateWinnerSendMessage(moneyA, moneyB) {
      ...
      await updateUser([message.author.id, message.guild.id], {
        ...
        console.log(
          db(`DB: S'ha actualitzat les monedes del user A ${message.author.username} a ${moneyA} correctament!`)
        )
      );
      ...
      await updateUser([other.id, message.guild.id], {
        ...
        console.log(db(`DB: S'ha actualitzat les monedes del user B ${other.username} a ${moneyB} correctament!`))
      );
    ...
    message.channel
      .send(
        '```' +
          other.username +
          " clica al ✅ per acceptar l'aposta o a la ❌ per cancel·lar!\n(15s per contestar...)```"
      )
      .then(async (msg) => {
        await msg.react('✅');
        await msg.react('❌');
