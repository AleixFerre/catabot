...
module.exports = {
	...
	async execute(message, args) {
		...
		const userData = await getUser(to.id, message.guild.id);
		...
		if (add < 1000) {
			// Si el que afegim es menor que el que val un nivell
			if (add + xp > 1000) {
				// Si la suma del que afegim i el que tenim, val un nivell
				sumLvl = 1; // Sumem un nivell
				sumXp = add - 1000; // Restem 1000 (el que val un nivell) al que sumem || xpactual + (add - 1000)
			} else {
				// Si no podem sumar cap nivell
				sumLvl = 0; // No sumem cap nivell
				sumXp = add; // Sumem la xp que podem
			}
		} else {
			// Si sumem mes d'un nivell en valor d'xp
			sumLvl = Math.floor(add / 1000); // Sumem els nivells que puguem
			const resta = add % 1000; // Agafem el que ens sobra per sumar
			if (resta + xp > 1000) {
				// Si el que ens queda mes el que tenim, fa un nivell
		...
		if (sumLvl > 0) {
			// Si es puja de nivell, avisa'm
			content += `\n${to.username}, has arribat al \`Nivell ${level}\``;
			if (level + sumLvl >= 200) {
				// Si just pujes a nivell 200, avisa
				content += `\n${to.username}, HAS ARRIBAT AL NIVELL MÀXIM!`;
			}
			if (rankIndexPrev !== rankIndexPost) {
				// Si pujes de rank, avisa
