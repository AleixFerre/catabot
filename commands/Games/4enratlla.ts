...
module.exports = {
	...
	async execute(message, _args, server) {
		...
		const depth = 5; // Torns que la IA prediu
		const scores = {
			// Objecte per predir el valor de cada jugada
			1: -Infinity,
			2: Infinity,
		};
		...
		const torns_per_actualitzar = 6; // Torns que han de passar per tornar a enviar un nou missatge
		let n_torn = 1; // Numero de torn actual
		const recompensa = 500; // Monedes que se li sumaran al guanyador
		...
		function generar_str_tauler() {
			...
			for (let i = 1; i <= tauler[0].length; i++) {
				str += emojis[i];
			}
		...
		async function mostrar_tauler() {
			const tauler_str = `**TORN ${n_torn} -- ${player.username}**
...
${generar_str_tauler()}`;
		...
		function primera_columna_valida() {
			for (let i = 0; i < tauler[0].length; i++) {
				if (!columna_plena(i)) {
					return i;
				}
			}
		...
		function comprovar_guanyador() {
			...
			for (let r = 0; r < 3; r++) {
				for (let c = 0; c < 7; c++) {
					if (chkLine(tauler[r][c], tauler[r + 1][c], tauler[r + 2][c], tauler[r + 3][c])) return tauler[r][c];
				}
			}
			...
			for (let r = 0; r < 6; r++) {
				for (let c = 0; c < 4; c++) {
					if (chkLine(tauler[r][c], tauler[r][c + 1], tauler[r][c + 2], tauler[r][c + 3])) return tauler[r][c];
				}
			}
			...
			for (let r = 0; r < 3; r++) {
				for (let c = 0; c < 4; c++) {
					if (chkLine(tauler[r][c], tauler[r + 1][c + 1], tauler[r + 2][c + 2], tauler[r + 3][c + 3])) return tauler[r][c];
				}
			}
...
`;
			...
			if (n_torn % torns_per_actualitzar === 0) {
				// Cada n torns
		...
		function countPieces(i, j, i2, j2, p1) {
			let pieces = 0;
		...
		function countDiagonal(i, j, direction, p1) {
			let pieces = 0;
			const h = 6;
			const w = 7;
			...
			for (let x = 0; x < 4; x++) {
				if (direction == 1) {
					if (i + x < h && j + x < w) {
						if (tauler[i + x][j + x] == p1) {
							pieces += 1;
						}
					}
				} else if (i + x < h && j - x < w && j - x > 0) {
					if (tauler[i + x][j - x] == p1) {
						pieces += 1;
					}
				}
		...
		function score_position(p1, p2) {
			...
			for (let i = 1; i < h; i++) {
				for (let j = 1; j < w; j++) {
					if (
						(countPieces(i, j, i + 4, j, p1) == 3 && countPieces(i, j, i + 4, j, 0) == 1) ||
						(countPieces(i, j, i, j + 4, p1) == 3 && countPieces(i, j, i, j + 4, 0) == 1) ||
						(countDiagonal(i, j, 0, p1) == 3 && countDiagonal(i, j, 0, 0) == 1) ||
						(countDiagonal(i, j, 1, p1) == 3 && countDiagonal(i, j, 1, 0) == 1)
					) {
						score += 1000;
					}

					if (
						(countPieces(i, j, i + 4, j, p1) == 2 && countPieces(i, j, i + 4, j, 0) == 2) ||
						(countPieces(i, j, i, j + 4, p1) == 2 && countPieces(i, j, i, j + 4, 0) == 2) ||
						(countDiagonal(i, j, 0, p1) == 2 && countDiagonal(i, j, 0, 0) == 2) ||
						(countDiagonal(i, j, 1, p1) == 2 && countDiagonal(i, j, 1, 0) == 2)
					) {
						score += 10;
					}

					if (
						(countPieces(i, j, i + 4, j, p1) == 1 && countPieces(i, j, i + 4, j, 0) == 3) ||
						(countPieces(i, j, i, j + 4, p1) == 1 && countPieces(i, j, i, j + 4, 0) == 3) ||
						(countDiagonal(i, j, 0, p1) == 1 && countDiagonal(i, j, 0, 0) == 3) ||
						(countDiagonal(i, j, 1, p1) == 1 && countDiagonal(i, j, 1, 0) == 3)
					) {
						score += 1;
					}

					if (
						(countPieces(i, j, i + 4, j, p2) == 3 && countPieces(i, j, i + 4, j, 0) == 1) ||
						(countPieces(i, j, i, j + 4, p2) == 3 && countPieces(i, j, i, j + 4, 0) == 1) ||
						(countDiagonal(i, j, 0, p2) == 3 && countDiagonal(i, j, 0, 0) == 1) ||
						(countDiagonal(i, j, 1, p2) == 3 && countDiagonal(i, j, 1, 0) == 1)
					) {
						score -= 1000;
					}

					if (
						(countPieces(i, j, i + 4, j, p2) == 2 && countPieces(i, j, i + 4, j, 0) == 2) ||
						(countPieces(i, j, i, j + 4, p2) == 2 && countPieces(i, j, i, j + 4, 0) == 2) ||
						(countDiagonal(i, j, 0, p2) == 2 && countDiagonal(i, j, 0, 0) == 2) ||
						(countDiagonal(i, j, 1, p2) == 2 && countDiagonal(i, j, 1, 0) == 2)
					) {
						score -= 10;
					}

					if (
						(countPieces(i, j, i + 4, j, p2) == 1 && countPieces(i, j, i + 4, j, 0) == 3) ||
						(countPieces(i, j, i, j + 4, p2) == 1 && countPieces(i, j, i, j + 4, 0) == 3) ||
						(countDiagonal(i, j, 0, p2) == 1 && countDiagonal(i, j, 0, 0) == 3) ||
						(countDiagonal(i, j, 1, p2) == 1 && countDiagonal(i, j, 1, 0) == 3)
					) {
						score -= 1;
					}
				}
		...
		function minimax(board, profunditat, isMaximizing, nr_moves, alpha, beta) {
			const result = comprovar_empat() ? 0 : comprovar_guanyador(); // Si empat, llavors 0; sino, comprovem guanyador
			...
			if (isMaximizing) {
				...
				for (let j = 0; j < tauler[0].length; j++) {
					if (!columna_plena(j)) {
						// Trobem el fons d'aquella columna
						const i = fons_columna(j);

						board[i][j] = 1;

						const score = minimax(board, profunditat - 1, false, nr_moves + 1, alpha, beta);
				...
				for (let j = 0; j < tauler[0].length; j++) {
					if (!columna_plena(j)) {
						...
						const score = minimax(board, profunditat - 1, true, nr_moves + 1, alpha, beta);
			...
			if (dificil) {
				...
				for (let j = 0; j < tauler[0].length; j++) {
					if (!columna_plena(j)) {
						...
						const score = minimax(tauler, depth, false, 1, -Infinity, Infinity);
						// Desfem el moviment
						tauler[i][j] = 0;
		...
		async function torn_jugador() {
			const filter = (m) => {
				if (isNaN(m.content) || m.author.id !== player.id || torn !== 1) {
					// Si no es un numero o no es el jugador actual o no es el seu torn
					return false;
				}

				const n = parseInt(m.content) - 1;
				if (n > tauler[0].length || n < 0) {
					// Si està fora del tauler
					return false;
				}

				if (columna_plena(n)) {
					// Si aquella fila està plena, no hi podem posar cap més fitxa
					return false;
				}
			...
			return await message.channel
				...
				.then((collected) => {
					return parseInt(collected.first().content - 1); // Ajustem a la notacio començant per 0
				})
				.catch(() => {
		...
		async function torn_jugador2() {
			const filter = (m) => {
				if (isNaN(m.content) || m.author.id !== player2.id || torn !== 2) {
					// Si no es un numero o no es el jugador actual o no es el seu torn
					return false;
				}
