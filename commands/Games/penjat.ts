...
module.exports = {
	...
	async execute(message, _args, server) {
		...
		const max_errades = 6; // Tens 6 errades maxim
		let torn = 1;
		...
		const acabat = await comenca_joc();
...
**RECORDA QUE HAS DE SUBTITUIR TOTS ELS ACCENTS PER LA SEVA RESPECTIVA LLETRA SENSE**[ p.e. àcid => acid ]`;
			...
			const demanar_msg = await message.author.send(contingut);
			const server_msg = await message.channel.send('El host està escollint la paraula...');

			const filter = (msg) => paraula_valida(msg.content.toLowerCase().split('')) && !msg.author.bot;
			const collected = await demanar_msg.channel
		...
		async function escriure_missatge() {
			...
			const embed_paraula = new MessageEmbed()
				.setColor(getColorFromCommand(TYPE))
...
Errades => ${errades}/${max_errades}`,
		...
		async function esperar_lletra(msg) {
			...
			const filter = (letterMsg) =>
				lletra_valida(letterMsg.content[0].toLowerCase()) && comprovar_host(letterMsg) && !letterMsg.author.bot;
		...
		async function comenca_joc() {
			...
			const missatge_paraula = await escriure_missatge();
			let resultat = -1; // -1 si ha de continuar la partida, 0 si perdut, 1 si guanyat
			while (resultat === -1) {
				torn++;
				const lletra = await esperar_lletra(missatge_paraula);
				if (lletra === -1) {
					return -1;
				}
				if (es_bona(lletra)) {
					resultat = descobrir_lletra(lletra);
				} else {
					resultat = fallar(lletra);
				}
		...
		async function acabar_joc(resultat) {
			...
			let emoji = '🏆';
			let desc = '';
			const link = `https://dlc.iec.cat/Results?DecEntradaText=${paraula}`;
			const questionLink = 'https://raw.githubusercontent.com/AleixFerre/CataBot/master/img/hangman/question.png';
			...
			if (resultat === 1) {
				...
				participants.forEach(async (participant) => {
					...
					const user = await getUser(participant.id, message.guild.id);
			...
			const embed_final = new MessageEmbed()
				.setColor(getColorFromCommand(TYPE))
